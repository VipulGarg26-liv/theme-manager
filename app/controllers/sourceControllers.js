...
async function createSource(req, res) {
    try {
        ...
        const postQuery = `INSERT INTO source(name, description, source_url) VALUES( $1, $2, $3) returning *`;
        const { rows } = await dbQuery.query(postQuery, [name, description, source_url]);
        console.log(rows);
        successMessage.data = rows[0];
        res.status(status.success).send(successMessage);
    } catch (error) {
...
async function updateSource(req, res) {
    try {
        ...
        const { name, description, source_url } = newSourceData;

        const updateQuery = `UPDATE source SET name=$1, description=$2, source_url=$3 WHERE id=$4 returning *`;
        const { rows: rows2 } = await dbQuery.query(updateQuery, [name, description, source_url, sourceId]);
